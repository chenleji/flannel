...
import (
	...
	_ "github.com/coreos/flannel/backend/yhvpc"
...
const (
	EnvFile        = "/etc/sysconfig/cni"
	EnvExtIfaceKey = "CNI_INTERFACE_NAME"
)
...
func getEnv(line string) {
	line = strings.TrimSpace(line)

	if strings.Contains(line, EnvExtIfaceKey) {
		strSped := strings.Split(line, "=")
		envVar[EnvExtIfaceKey] = strSped[1]
	}
}
